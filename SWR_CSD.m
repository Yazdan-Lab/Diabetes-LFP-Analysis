    ...
    for j = grouping
        ...
        for k = 1:size(SWRLTDIdx,2)
            if ~isempty(SWRLTDIdx(k).R) % makes sure ripple occured during this period
                ...
                LTDevents = SWRevents(SWRLTDIdx(k).R,1);
                LTDevents = LTDevents(LTDevents >= 625);
                LTDevents = LTDevents(LTDevents <= length(LFP)-1250);
                
                temp_CSD = zeros(1876,12,length(LTDevents));
                for l = 1:length(LTDevents)
                    temp_CSD(:,:,l) = CSD(LFP(LTDevents(l)-625:LTDevents(l)+1250,chans(j)-5:chans(j)+6),1250,1e-4);
                    pause(0.7)
                    disp(num2str(j))
                end
                
                if isempty(SwrFullCsd)
                    SwrFullCsd = temp_CSD;
                else
                    SwrFullCsd = cat(3,SwrFullCsd,temp_CSD);
                end
            end % for k SWRLTDIdx
